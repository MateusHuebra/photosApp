...
abstract class LoggedController extends Controller {
	...
	function __construct() {
		if(!(new \Service\Authentication())->isLogged()) {
			$this->redirect('authentication/login', \Service\Translation::get('toasts.youShouldBeLoggedInToSeeThatPage'));
		}
	}
	...
	protected function uploadPicture(string $targetFolder, string $targetName, bool $square = false, string $old = null) {
		...
		if(!isset($_FILES['picture']) || empty($_FILES['picture']['name'])) {
			throw new \Exception\UploadPictureError(\Service\Translation::get('toasts.noFileWasSent'));
		}
		...
		if($fileExtension != 'png' && $fileExtension != 'jpg' && $fileExtension != 'jpeg') {
			throw new \Exception\UploadPictureError(\Service\Translation::get('toasts.yourFileIsNotAPNGJPGorJPEG'));
		}
		if($_FILES["picture"]["size"] > 10000000) {
			throw new \Exception\UploadPictureError(\Service\Translation::get('toasts.yourFIleIsTooLarge'));
		}
		if($square && (getimagesize($_FILES["picture"]["tmp_name"])[0] != getimagesize($_FILES["picture"]["tmp_name"])[1])) {
			throw new \Exception\UploadPictureError(\Service\Translation::get('toasts.yourImageIsNotASquare'));
		}
		...
		if(move_uploaded_file($_FILES["picture"]["tmp_name"], $target)){
			...
			throw new \Exception\UploadPictureError(\Service\Translation::get('toasts.somethingGoneWrong'));
		}
